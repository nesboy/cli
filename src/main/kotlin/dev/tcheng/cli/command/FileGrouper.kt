...
import dev.tcheng.cli.model.TransferMode
...
object FileGrouper :
    ...
    private fun processFiles(targetPath: Path, directoryName: String, files: List<File>) {
        ...
        files.forEach {
            val target = Path("""$destinationDirectoryPath\${it.name}""")
            val options = arrayOf(StandardCopyOption.REPLACE_EXISTING)

            if (transferMode == TransferMode.COPY) {
                it.toPath().copyTo(target, options = options)
            } else {
                it.toPath().moveTo(target, options = options)
            }
